...
class PHPQRCode
{
    ...
    public function generate ($data)
    {
        ...
        // 合拼临时二维码图片与logo图片
        $new_qrcode_file = $this->add_logo($tmp_qrcode_file);
        // 删除临时二维码图片
        if ($tmp_qrcode_file != '' && file_exists($tmp_qrcode_file)) {
            unlink($tmp_qrcode_file);
        }

        return file_exists($new_qrcode_file) ? $new_qrcode_file : '';
    ...
    {
        // 临时二维码图片
        $tmp_qrcode_file = $this->getConfigPath() . '/tmp_qrcode_' . time() . mt_rand(100, 999) . '.png';
        // 创建临时二维码
...
//        $this->create_dirs(dirname($this->_config['dest_file']));
        $createFile = $this->getConfigPath().'/'.$this->_config['dest_file'];
        ...
        if (file_exists($this->_config['logo'])) {
            ...
            switch ($dest_ext) {
                case 1:
                    imagegif($dest_img, $createFile, $this->_config['quality']);
                    break;
                case 2:
                    imagejpeg($dest_img,$createFile, $this->_config['quality']);
                    break;
                case 3:
                    imagepng($dest_img, $createFile, (int)(($this->_config['quality'] - 1) / 10));
                    break;
            }
    ...
    {
        $filePath = getenv('QR_PATH') ? getenv('QR_PATH') : '/tmp/qr';
        $this->create_dirs($filePath);
